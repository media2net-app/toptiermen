-- Brotherhood Database Schema - Fixed Version
-- Only adds missing columns to existing tables

-- Add missing columns to existing brotherhood_groups table
ALTER TABLE brotherhood_groups 
ADD COLUMN IF NOT EXISTS status VARCHAR(20) DEFAULT 'active' CHECK (status IN ('active', 'inactive', 'archived'));

-- Add missing columns to existing brotherhood_events table  
ALTER TABLE brotherhood_events
ADD COLUMN IF NOT EXISTS event_type VARCHAR(50) DEFAULT 'meetup' CHECK (event_type IN ('meetup', 'workshop', 'call', 'social')),
ADD COLUMN IF NOT EXISTS duration_minutes INTEGER DEFAULT 120,
ADD COLUMN IF NOT EXISTS is_online BOOLEAN DEFAULT false,
ADD COLUMN IF NOT EXISTS max_attendees INTEGER;

-- Create brotherhood_group_members table (if not exists)
...
-- Create brotherhood_event_attendees table (if not exists)
...
-- Create brotherhood_group_posts table (if not exists)
...
-- Create brotherhood_group_activity table (if not exists)

...
export default function PlanBuilder({ isOpen, onClose, plan, foodItems = [], onSave }: PlanBuilderProps) {
  ...
  // Standaard profiel: Man 40 jaar, 100kg, 190cm, Staand (Matig actief)
  const getStandardProfile = (fitnessGoal: 'droogtrainen' | 'spiermassa' | 'onderhoud' = 'spiermassa', isCarnivore: boolean = false) => {
    ...
    // Activiteitniveau voor "Staand (Matig actief)" = 1.3
    const activityMultiplier = 1.3;
    const tdee = standardWeight * 22 * activityMultiplier;
    
    // Nieuwe calorie berekeningen gebaseerd op onderhoud
    const configs = {
      droogtrainen: {
        calories: Math.round(tdee - 500), // -500 kcal van onderhoud
      ...
      spiermassa: {
        calories: Math.round(tdee + 400), // +400 kcal van onderhoud
        protein: baseProtein, // 2.2x of 3.0x gewicht afhankelijk van plan type
        carbs: isCarnivore ? 10 : 350,     // Minimaal voor carnivoor, hoog voor normaal
        description: 'Spiergroei en krachttoename'
      },
      onderhoud: {
        calories: Math.round(tdee), // Onderhoudscalorieën = dagelijkse calorieën
        protein: baseProtein, // 2.2x of 3.0x gewicht afhankelijk van plan type
            ...
            <div className="space-y-6">
              ...
              <div className="bg-[#232D1A] rounded-lg p-4 border border-[#3A4D23] mb-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                  ...
                  <div className="bg-[#181F17] rounded-lg p-3 border border-[#3A4D23]">
                    <div className="text-[#8BAE5A] font-semibold text-sm">Staand (Matig actief)</div>
                    <div className="text-[#B6C948] text-xs">Staand werk, matige beweging</div>
                  </div>
                </div>
                <div className="mt-4 text-xs text-[#B6C948] space-y-2">
                  ...
                  <div className="bg-[#0F150E] rounded p-3 border border-[#2A3520] space-y-1">
                    <div><span className="text-[#8BAE5A]">Formule:</span> gewicht × 22 × activiteitniveau</div>
                    <div className="text-xs text-[#B6C948] ml-2">
                      <div>• Zittend (Licht actief): 1.1</div>
                      <div>• Staand (Matig actief): 1.3</div>
                      <div>• Lopend (Zeer actief): 1.6</div>
                    </div>
                    <div><span className="text-[#8BAE5A]">Dagelijkse calorieën:</span> 100 × 22 × 1.3 (Staand) = <span className="text-[#B6C948] font-semibold">2,860 kcal</span></div>
                    <div className="border-t border-[#3A4D23] pt-2 mt-2">
                      <div><span className="text-[#8BAE5A]">🔥 Droogtrainen:</span> 2,860 - 500 = <span className="text-yellow-400 font-semibold">2,360 kcal</span> (min -500 kcal van onderhoud)</div>
                      <div><span className="text-[#8BAE5A]">⚖️ Onderhoud:</span> <span className="text-green-400 font-semibold">2,860 kcal</span> (dagelijkse calorieën)</div>
                      <div><span className="text-[#8BAE5A]">💪 Spiermassa:</span> 2,860 + 400 = <span className="text-blue-400 font-semibold">3,260 kcal</span> (+400 kcal van onderhoud)</div>
                    </div>
                  </div>
                  <div className="text-center">
                    📊 Alle voedingsplannen zijn gebaseerd op profieldata: 100kg - Staand (Matig actief)
                  </div>
                </div>
              </div>
                          ...
                          <div className="bg-[#232D1A] rounded-lg p-4 border border-[#3A4D23]">
                            ...
                            <div className="text-xs text-center">
                              <div className="text-[#B6C948] mb-1">
                                💡 Profiel: 40 jaar, 100kg, 190cm, Staand (Matig actief)
                              </div>

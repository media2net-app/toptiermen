...
export async function POST(request: NextRequest) {
  try {
    console.log('📝 Direct test notes API called');
    ...
    // Try to insert the note directly
    ...
    if (error) {
      ...
      if (error.message && error.message.includes('relation "public.test_notes" does not exist')) {
        ...
        if (retryError) {
          ...
          return NextResponse.json({ 
            error: 'Failed to save note after table creation',
...
export async function GET(request: NextRequest) {
  try {
    console.log('📝 Direct test notes GET API called');

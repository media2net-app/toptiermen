...
export async function POST() {
  ...
  try {
    console.log('💼 Improving Zakelijk targeting with business and leadership interests...');

    // Define the interests to add for Zakelijk
    const zakelijkInterests = [
      { name: 'Ondernemers', searchTerm: 'Ondernemers' },
      { name: 'Business professionals', searchTerm: 'Business professionals' },
      { name: 'Carrière', searchTerm: 'Carrière' },
      { name: 'Netwerken', searchTerm: 'Netwerken' },
      { name: 'Leadership', searchTerm: 'Leadership' },
      { name: 'Management', searchTerm: 'Management' },
      { name: 'Professional development', searchTerm: 'Professional development' }
    ];
    ...
    for (const interest of zakelijkInterests) {
    ...
    console.log(`📋 Found ${foundInterests.length} interests for Zakelijk`);

    // Get all Zakelijk ad sets
    ...
    // Filter to only show Zakelijk ad sets
    const zakelijkAdSets = data.data.filter((adset: any) => 
      adset.name && adset.name.includes('TTM') && adset.name.includes('Zakelijk')
    );

    console.log(`📋 Found ${zakelijkAdSets.length} Zakelijk ad sets to update`);
    ...
    // Update each Zakelijk ad set
    for (const adset of zakelijkAdSets) {
      try {
        ...
        if (interestsToAdd.length === 0) {
          console.log(`✅ Ad set ${adset.name} already has all Zakelijk interests`);
          continue;
        }
    ...
    console.log(`✅ Improved ${successCount} Zakelijk ad sets successfully`);
    console.log(`❌ ${errorCount} ad sets failed to update`);

    return NextResponse.json({
      success: true,
      data: {
        total_zakelijk_ad_sets: zakelijkAdSets.length,
    ...
    console.error('❌ Error improving Zakelijk targeting:', error);
    return NextResponse.json(
      { success: false, error: 'Failed to improve Zakelijk targeting' },
      { status: 500 }
    );
  }
}

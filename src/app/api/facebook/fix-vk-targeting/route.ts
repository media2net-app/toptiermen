...
export async function POST() {
  ...
  try {
    console.log('🔧 Fixing VK targeting by replacing with Social media targeting...');
    ...
    console.log(`📋 Found ${ttmAdSets.length} TTM ad sets to check for VK targeting`);
    ...
    for (const adset of ttmAdSets) {
      try {
        ...
        // Create new targeting without VK and with Social media
        const newTargeting = {
          ...targeting,
          interests: [
            ...targeting.interests.filter((interest: any) => 
              interest.name !== 'VK' && interest.id !== '6002714396372'
            ),
            {
              name: 'Sociale media (online media)',
              id: '6002714396372' // This is the ID for Social media
            }
          ]
        };
    ...
    console.log(`✅ Fixed ${successCount} ad sets successfully`);
    ...
    console.error('❌ Error fixing VK targeting:', error);
    return NextResponse.json(
      { success: false, error: 'Failed to fix VK targeting' },
      { status: 500 }
    );
  }
}

      ...
      {showTooltip && (
        <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 w-64 bg-[#1A2317] border border-[#3A4D23] rounded-lg p-3 shadow-xl z-50">
          <div className="text-center">
            ...
            <div className="text-xs text-[#FFD700] mb-2">Zeldzaamheid: {badge.rarity_level}</div>
            <div className="text-xs text-[#8BAE5A] mb-2">XP Beloning: {badge.xp_reward}</div>
            {badge.requirements && (
...
export default function BadgesRangen() {
  ...
  return (
    <div className="w-full bg-[#232D1A] min-h-screen">
      ...
      <div className="relative py-6 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-6">
            <h1 className="text-4xl md:text-5xl font-bold text-white mb-3">Badges & Rangen</h1>
      ...
      {/* Tab Content (simplified counts, CTA removed) */}
      <div className="px-4">
        <div className="max-w-7xl mx-auto">
          {activeTab === 'requirements' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {ranks.map((rank: any, index: number) => (
                  <div
                    key={rank.id}
                    className={`bg-gradient-to-br from-[#1A2317] to-[#232D1A] rounded-xl p-6 border-2 transition-all duration-300 hover:scale-[1.02] ${
                      index === 0 ? 'border-[#FFD700] shadow-lg shadow-[#FFD700]/20' : 'border-[#3A4D23]/30'
                    }`}
                  >
                    <div className="text-center mb-4">
                      <div className="text-4xl text-[#FFD700] mb-2">{getIconComponent(rank.icon_name)}</div>
                            ...
                            <span
                              className={`px-2 py-1 rounded text-xs font-semibold ${
                                badge.rarity_level === 'legendary'
                                  ...
                                  : badge.rarity_level === 'epic'
                                  ? 'bg-[#8BAE5A] text-white'
                                  : badge.rarity_level === 'rare'
                    ...
                    >
                      ...
                      <div
                        className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg ${
                          index === 0
                            ? 'bg-gradient-to-r from-[#FFD700] to-[#FFA500] text-[#181F17]'
                            : index === 1
                            ? 'bg-gradient-to-r from-[#C0C0C0] to-[#E5E5E5] text-[#181F17]'
                            : index === 2
                            ? 'bg-gradient-to-r from-[#CD7F32] to-[#B8860B] text-white'
                            : 'bg-[#3A4D23] text-[#8BAE5A]'
                        }`}
                      >
                        {index + 1}
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {member.id === user?.id && (
                            <span className="px-2 py-1 bg-[#8BAE5A] text-[#181F17] text-xs font-semibold rounded">Jij</span>
                          )}
                        </div>
                        <div className="flex items-center gap-4 text-sm text-[#8BAE5A]">
                          {leaderboardCategory === 'xp' && (
                            <div className="flex items-center gap-1">
                              <FaStar className="text-[#FFD700]" />
                              <span>{member.xp?.[leaderboardPeriod] ?? 0} XP</span>
                            </div>
                          )}
                          {leaderboardCategory === 'badges' && (
                            <div className="flex items-center gap-1">
                              <FaMedal className="text-[#8BAE5A]" />
                              <span>{member.badges?.[leaderboardPeriod] ?? 0} badges</span>
                            </div>
                          )}
                          {leaderboardCategory === 'levels' && (
                            <div className="flex items-center gap-1">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <span>Lid sinds {member.joinDate ? new Date(member.joinDate).getFullYear() : '-'}</span>
                          </div>
                        </div>
                      </div>

...
export default function Brotherhood() {
  ...
  const fetchBrotherhoodData = async () => {
    try {
      ...
      const res = await fetch('/api/brotherhood/overview', { cache: 'no-cache', signal: controller.signal });
      clearTimeout(t);
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();
      const formattedTopics: Topic[] = (data.topics || []).map((topic: any) => ({
        id: topic.id,
        title: topic.title,
        author: topic.author,
        time: formatTimeAgo(topic.created_at)
      }));
      const formattedConnections: Connection[] = (data.profiles || []).map((p: any) => ({
        id: p.id,
        name: p.name,
        avatar: p.avatar,
        rank: p.rank

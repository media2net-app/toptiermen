...
async function createTrainingSchemaProgressionSystemFinal() {
  try {
    ...
    for (const [cycleKey, cycleData] of Object.entries(PROGRESSION_SYSTEM)) {
      ...
      for (const [styleKey, styleData] of Object.entries(TRAINING_STYLES)) {
        ...
        // Create schema with existing categories
        const { data: schema, error: schemaError } = await supabase
          ...
          .insert({
            ...
            equipment_type: cycleData.category === 'Outdoor' ? 'bodyweight' : 'gym'
          })
          .select()
          .single();
    ...
    for (const program of specializedPrograms) {
      ...
      const { data: schema, error: schemaError } = await supabase
        ...
        .insert({
          ...
          equipment_type: program.category === 'Outdoor' ? 'bodyweight' : 'gym'
        })
        .select()
        .single();
    ...
    // 3. Create 6-month progression overview
    console.log('\n📊 Creating 6-month progression overview...');
    
    const progressionOverview = {
      name: '6-Month Progression Overview',
      description: `Complete 6-maand progressie systeem voor optimale resultaten:

**Maand 1-2: Foundation & Muscle Building**
- Focus: Spiergroei en correcte uitvoering
- Reps: 8-12, Rust: 90s
- Doel: Sterke basis opbouwen

**Maand 3-4: Strength & Power Development**
- Focus: Kracht en power
- Reps: 3-6, Rust: 180s
- Doel: Neurologische adaptatie

**Maand 5-6: Endurance & Conditioning**
- Focus: Uithouding en conditionering
- Reps: 15-20, Rust: 45s
- Doel: Cardiovasculaire fitness
...
⚠️ **BELANGRIJK:** We adviseren ten alle tijden om het aantal herhalingen te doen tot spierfalen moment voor optimale resultaten.`,
      ...
      difficulty: 'Advanced',
      status: 'published',
      training_goal: 'progression',
      rep_range: '3-20',
      rest_time_seconds: 45,
      equipment_type: 'gym'
    };

    const { data: overviewSchema, error: overviewError } = await supabase
      .from('training_schemas')
      .insert(progressionOverview)
      .select()
      .single();

    if (overviewError) {
      console.log(`⚠️ Could not create progression overview:`, overviewError.message);
    } else {
      console.log(`✅ Created progression overview: ${progressionOverview.name}`);
    }
    ...
    console.log('✅ 1 progression overview');
    ...
    console.log('✅ Compatible with existing category constraints');
...
createTrainingSchemaProgressionSystemFinal();

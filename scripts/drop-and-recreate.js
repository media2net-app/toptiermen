...
async function dropAndRecreateTable() {
  try {
    console.log('🚀 Drop en recreate van nutrition_plans tabel...');
    
    // First, try to drop the table completely
    console.log('🗑️  Proberen tabel te droppen...');
    
    // Try to drop via a direct approach
    const dropSQL = `
    ...
    if (dropError) {
      console.log('⚠️  Kon tabel niet droppen via RPC. Probeer handmatige drop.');
    ...
    if (createError) {
      console.log('❌ Kon tabel niet aanmaken via RPC. Probeer handmatige creatie.');
      console.log('📋 Ga naar: https://supabase.com/dashboard/project/wkjvstuttbeyqzyjayxj/sql');
      console.log('📋 Voer dit uit:');
      console.log(`
        CREATE TABLE nutrition_plans (
    ...
    for (const plan of hardcodedPlans) {
      ...
      if (insertError) {
        ...
        console.log(`✅ Plan ${plan.name} succesvol ingevoegd`);
        successCount++;
      }
    }

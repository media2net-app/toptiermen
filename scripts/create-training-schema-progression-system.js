...
// Exercise variations for each progression phase
const EXERCISE_VARIATIONS = {
  'spiermassa': {
    'compound': ['Bench Press', 'Squats', 'Deadlifts', 'Overhead Press', 'Barbell Rows'],
    'isolation': ['Dumbbell Flyes', 'Leg Extensions', 'Leg Curls', 'Lateral Raises', 'Bicep Curls'],
    'accessory': ['Cable Flyes', 'Hip Thrusts', 'Face Pulls', 'Tricep Pushdowns', 'Hammer Curls']
  },
  'kracht': {
    'compound': ['Bench Press', 'Squats', 'Deadlifts', 'Overhead Press', 'Barbell Rows'],
    'power': ['Power Cleans', 'Snatch', 'Box Jumps', 'Medicine Ball Throws', 'Plyometric Push-ups'],
    'accessory': ['Good Mornings', 'Romanian Deadlifts', 'Military Press', 'Pull-ups', 'Dips']
  },
  'uithouding': {
    'compound': ['Bodyweight Squats', 'Push-ups', 'Pull-ups', 'Lunges', 'Burpees'],
    'cardio': ['Mountain Climbers', 'Jumping Jacks', 'High Knees', 'Butt Kicks', 'Plank Walks'],
    'circuit': ['Circuit Training', 'Tabata Intervals', 'AMRAP Sets', 'EMOM Workouts', 'Ladder Drills']
  }
};

async function createTrainingSchemaProgressionSystem() {
  try {
    console.log('🏋️ Creating comprehensive training schema progression system...\n');
    ...
    for (const [cycleKey, cycleData] of Object.entries(PROGRESSION_SYSTEM)) {
      ...
      for (const [styleKey, styleData] of Object.entries(TRAINING_STYLES)) {
        ...
        const schemaDescription = `${styleData.description} ${cycleData.description} ${cycleData.frequency}. Rep range: ${cycleData.rep_range}, Rust: ${cycleData.rest_time}s. ⚠️ **BELANGRIJK:** We adviseren ten alle tijden om het aantal herhalingen te doen tot spierfalen moment voor optimale resultaten.`;
        ...
        const { data: schema, error: schemaError } = await supabase
          ...
          .insert({
            ...
            difficulty: cycleData.intensity === 'high' ? 'Advanced' : cycleData.intensity === 'moderate' ? 'Intermediate' : 'Beginner',
            status: 'published',
            ...
            equipment_type: 'gym',
            progression_cycle: cycleKey,
            training_style: styleKey,
            intensity_level: cycleData.intensity,
            volume_level: cycleData.volume
          })
          .select()
          .single();
    ...
    const specializedPrograms = [
      {
        ...
        rep_range: '5-8',
        rest_time: 120,
        days: ['Power & Speed', 'Strength & Stability', 'Agility & Coordination', 'Recovery & Mobility']
      },
      {
        ...
        rep_range: '8-15',
        rest_time: 75,
        days: ['Upper Body Focus', 'Lower Body Focus', 'Full Body Circuit', 'Active Recovery']
      },
      {
        ...
        rep_range: '6-12',
        rest_time: 90,
        days: ['Sport Skills', 'Strength Foundation', 'Conditioning', 'Recovery & Mobility']
      }
    ];
    ...
    for (const program of specializedPrograms) {
      ...
      for (let dayIndex = 0; dayIndex < program.days.length; dayIndex++) {
        const dayName = program.days[dayIndex];
    ...
    console.log('✅ Varied training styles and intensities');
...
function generateSpecializedExercises(dayName, program) {
  ...
  } else if (dayName.includes('Circuit') || dayName.includes('Conditioning')) {
    exercises.push(
...
createTrainingSchemaProgressionSystem();

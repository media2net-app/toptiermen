...
async function createTableDirectly() {
  try {
    console.log('🚀 Directe tabel creatie via Supabase client...');
    
    // First, let's check if the table already exists
    console.log('🔍 Controleren of tabel al bestaat...');
    ...
    if (checkError) {
      console.log('⚠️  Kon tabel check niet uitvoeren, ga door...');
    } else if (existingTables && existingTables.length > 0) {
      console.log('✅ Tabel nutrition_plans bestaat al!');
    } else {
      console.log('📝 Tabel bestaat nog niet, ga door met creatie...');
    }
    ...
    console.log('📝 Proberen tabel te creëren via insert...');
    
    const testRecord = {
      plan_id: 'test-creation',
    ...
    if (error) {
      if (error.message.includes('relation "nutrition_plans" does not exist')) {
        console.log('❌ Tabel bestaat nog niet. Probeer handmatige creatie...');
        
        // Try to create via a different approach - using a function call
        console.log('📝 Proberen via function call...');
        ...
        if (funcError) {
          console.log('⚠️  Kon function niet aanmaken, probeer directe SQL...');
          ...
          if (directError) {
            console.log('❌ Kon tabel niet aanmaken via directe SQL. Handmatige creatie vereist.');
          ...
          if (execError) {
            console.log('❌ Kon tabel niet aanmaken via function. Handmatige creatie vereist.');
            return;
          }
        }
        ...
        console.error('❌ Andere fout bij tabel creatie:', error);
        return;
      }
    } else {
      console.log('✅ Tabel bestaat al en test insert succesvol!');
    }
    ...
    if (data) {
      await supabase
        .from('nutrition_plans')
        .delete()
        .eq('plan_id', 'test-creation');
      console.log('🧹 Test record opgeruimd');
    }
    
    // Now try to insert the actual plans
    console.log('📝 Invoegen van echte voedingsplannen...');

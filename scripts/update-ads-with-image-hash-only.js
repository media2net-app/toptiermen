...
async function updateAdsWithImageHashOnly() {
  console.log('🎬 Updating ads with image_hash only...\n');
  ...
  try {
    ...
    for (const creative of creativesToUpdate) {
      ...
      // Create new creative with video_data structure using only image_hash
      try {
            ...
            body: JSON.stringify({
              ...
              object_story_spec: {
                ...
                video_data: {
                  ...
                  image_hash: knownImageHash // Only use image_hash, not image_url
                }
              }
            })
          }
...
updateAdsWithImageHashOnly().catch(console.error);

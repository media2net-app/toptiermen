...
// Get Facebook configuration from environment
const FACEBOOK_ACCESS_TOKEN = process.env.FACEBOOK_ACCESS_TOKEN;
const FACEBOOK_AD_ACCOUNT_ID = process.env.FACEBOOK_AD_ACCOUNT_ID || '1465834431278978';
...
async function createAllCampaignsToFacebook() {
  console.log('🚀 Creating All 4 Campaigns to Facebook...\n');
  ...
  try {
    ...
    for (const [campaignKey, campaignData] of Object.entries(ALL_CAMPAIGNS)) {
      ...
      const campaignVideos = videosByAudience[campaignData.name.includes('Algemeen') ? 'Algemeen' : 
                                            campaignData.name.includes('Jongeren') ? 'Jongeren' :
                                            campaignData.name.includes('Vaders') ? 'Vaders' : 'Zakelijk'] || [];

              // Add video URLs to ad sets
        const campaignWithVideos = {
          ...campaignData,
          ad_account_id: FACEBOOK_AD_ACCOUNT_ID,
        ad_sets: campaignData.ad_sets.map((adSet, index) => ({
          ...adSet,
          video_url: campaignVideos[index]?.file_path,
          video_name: campaignVideos[index]?.name
        }))
      };

      console.log(`📊 Campaign: ${campaignWithVideos.name}`);
      console.log(`💰 Total Budget: €${campaignWithVideos.campaign_daily_budget}`);
      console.log(`📊 Ad Sets: ${campaignWithVideos.ad_sets.length}`);

      try {
        const response = await fetch('http://localhost:3000/api/facebook/create-multi-adset-campaign', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(campaignWithVideos)
        });
    ...
    console.log('3. Activate campaigns when ready');
    console.log('4. Monitor performance and adjust as needed');
    console.log('5. Focus on email collection for the prelaunch waitlist');

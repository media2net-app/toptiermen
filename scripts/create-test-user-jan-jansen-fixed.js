...
async function createTestUserJanJansenFixed() {
  try {
    console.log('🧪 Creating comprehensive test user: Jan Jansen (Fixed Schema)\n');
    ...
    // 2. Create profile (using correct schema)
    ...
    // 5. Assign missions (using correct schema)
    ...
    // 6. Assign nutrition plan (using correct schema)
    ...
    const { error: nutritionError } = await supabase
      ...
      .insert({
        ...
        is_active: true,
        user_data: { preferences: ['high_protein', 'low_sugar'] }
      });
    ...
    // 7. Create user goals (using correct schema)
    ...
    // 8. Create user habits (using correct schema)
    console.log('\n🔄 Step 8: Creating user habits...');
    const habits = [
      { title: 'Dagelijkse workout', description: 'Train elke dag minimaal 30 minuten', frequency: 'daily' },
      { title: 'Meditatie', description: 'Mediteer elke ochtend 10 minuten', frequency: 'daily' },
      { title: 'Water drinken', description: 'Drink 2L water per dag', frequency: 'daily' }
    ];
    ...
    for (const habit of habits) {
      ...
      if (habitError) {
        console.log(`❌ Error creating habit ${habit.title}:`, habitError.message);
      } else {
    ...
    // 10. Create daily progress entries (using correct schema)
